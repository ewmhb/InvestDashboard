...
function setupDashboard() {
  ...
  [['투자심리','fearGreedValue','fearGreedAsOf'],['Equity P/C','equityPcValue','optionSentimentAsOf'],['유동성 점수','liquidityScore','liquidityWeeklyAsOf']].forEach(([label,id,dateId])=>{const card=radarNode('article');const value=radarNode('strong','—'),date=radarNode('small','확인 중');card.append(radarNode('span',label),value,date);compact.append(card);const source=risk.querySelector("#"+id),dateSource=document.getElementById(dateId);if(source){const sync=()=>{value.textContent=source.textContent;date.textContent=dateSource?.textContent||'상세에서 기준일 확인';};const observer=new MutationObserver(sync);observer.observe(source,{childList:true,subtree:true,characterData:true});if(dateSource&&dateSource!==source)observer.observe(dateSource,{childList:true,subtree:true,characterData:true});sync();}});
