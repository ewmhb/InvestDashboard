(function(root){
  ...
  function parse(payload){
    ...
    if(!Array.isArray(rows))throw Error('금리 응답 없음');
  ...
  function mount(){
    ...
    const panel=el('div',undefined,'treasury-yields');panel.id='treasuryYields';panel.setAttribute('aria-label','미국채 장중 금리');
    ...
    async function refreshQuotes(){
      ...
      try{
        ...
        for(const q of quotes){
          ...
          c.time.textContent=q.realtime?'':'지연';c.card.title='현재 금리 · CNBC · '+q.asOf+' · 1분마다 조회';
        }
        lastSuccess=Date.now();panel.classList.remove('refresh-error');
        const missing=quotes.filter(q=>q.unavailable).length;
        status.textContent=(missing?'일부 만기 조회 실패 · ':'')+'CNBC · Tradeweb 금리 · 1분마다 갱신 · 확인 '+checked()+' KST · 시세 시각은 공급자 기준(미 동부)';
      }catch{
