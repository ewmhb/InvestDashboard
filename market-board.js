...
async function setupMarketBoard() {
  ...
  drawMarket();
  function drawLabor(){
    const laborFallback={ICSA:{rows:[['2026-07-25',198000],['2026-08-01',200000],['2026-08-08',212000],['2026-08-15',207000],['2026-08-22',204000],['2026-08-29',207000],['2026-09-05',206000],['2026-09-12',196000]].map(([date,value])=>({date,value}))},CCSA:{rows:[['2026-08-08',1796000],['2026-08-15',1771000],['2026-08-22',1775000],['2026-08-29',1769000],['2026-09-05',1730000]].map(([date,value])=>({date,value}))},UNRATE:{rows:[['2026-07-01',4.1],['2026-08-01',4.1]].map(([date,value])=>({date,value}))},PAYEMS:{rows:[['2026-07-01',158913],['2026-08-01',159075]].map(([date,value])=>({date,value}))}};
    const series={...laborFallback,...(data.macro?.series||{})},rows=id=>(series[id]?.rows||[]).filter(row=>Number.isFinite(row.value)),latest=id=>rows(id).at(-1),change=(id,count=1)=>{const values=rows(id),now=values.at(-1),prior=values.at(-1-count);return now&&prior?now.value-prior.value:null;},average=(id,prior=false)=>{const values=rows(id),sample=prior?values.slice(-8,-4):values.slice(-4);return sample.length===4?sample.reduce((sum,row)=>sum+row.value,0)/4:null;};
    const laborPeriod=date=>date?date.slice(0,4)+'년 '+Number(date.slice(5,7))+'월분':'기준월 미확보',laborRelease=date=>{const release=new Date(date+'T00:00:00Z');release.setUTCMonth(release.getUTCMonth()+1,1);while(release.getUTCDay()!==5)release.setUTCDate(release.getUTCDate()+1);return release.toISOString().slice(0,10);};
    ...
    laborBody.replaceChildren(boardCard('신규 실업수당 청구',Math.round(initial.value/1000).toLocaleString('en-US')+'천 건',initialText+' · 4주 평균 '+(initialAvg===null?'—':Math.round(initialAvg/1000).toLocaleString('en-US')+'천 건')+' · 기준 '+initial.date),boardCard('계속 실업수당 청구',(continued.value/1000000).toFixed(2)+'백만 건',continuedText+' · 기준 '+continued.date),boardCard('실업률',unemployment.value.toFixed(1)+'%',unemploymentText+' · '+laborPeriod(unemployment.date)+' · 발표 '+laborRelease(unemployment.date)),boardCard('비농업고용자 수',(payroll.value/1000).toFixed(1)+'백만 명',payrollText+' · '+laborPeriod(payroll.date)+' · 발표 '+laborRelease(payroll.date)),radarNode('p','출처: 미국 노동부·BLS, FRED 경유 · 주간 청구수당은 계절조정 기준이며 단일 주간 수치보다 4주 평균 추세를 우선합니다.','board-subtitle'));
  }
  drawLabor();
