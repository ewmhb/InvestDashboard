...
function boardSignals(data,now=Date.now()) {
  ...
  return [
    signal('유동성',l===null?'미확보':macroSigned(l/1000,'$B / 4주 전 대비'),liq.at(-1)?.date,l===null?null:l>10000?1:l< -10000?-1:0,'지급준비금 4주 전 대비 ±$10B',14),
...
function boardNarrative(data,signals,now=Date.now()) {
  ...
  if(liq.score!==null){
    ...
    for(const [id,label,mult] of [['WTREGEN','TGA',0.001],['RRPONTSYD','ON RRP',1]]){
      ...
      if(delta!==null&&boardFresh(series.at(-1)?.date,14,now))context.push(label+'는 4주 전 대비 '+Math.abs(delta*mult*10).toFixed(0)+'억 달러 '+(delta>0?'늘어 유동성 흡수 방향':delta<0?'줄어 유동성 공급 방향':'변화가 없는 상태'));
    }
...
async function setupMarketBoard() {
  ...
  const liquidity=boardSection('LIQUIDITY','현재 잔고와 1주 전 · 4주 전 · 13주 전 대비 증감 · 단위 $B (10억 달러)','liquidityBoard');
