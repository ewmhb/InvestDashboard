...
(async()=>{
  ...
  assert.equal(run('peerThemeFor("IONQ").name'),'양자컴퓨팅');
  assert.equal(run('peerThemeFor("RKLB").name'),'우주·위성');
  assert.equal(run('peerThemeFor("AAPL")'),null);
  for(const ticker of ['RGTI','QBTS','QUBT']) profiles[ticker]={ticker,country:'US',currency:'USD',finnhubIndustry:'Technology',marketCapitalization:100};
  const before=calls.length;
  const themed=await run('loadSectorPeerData("IONQ","test-key")');
  assert.deepEqual(Array.from(themed.rows,x=>x.ticker),['RGTI','QBTS','QUBT']);
  assert.ok(calls.slice(before).every(call=>!new URL(call.url).pathname.endsWith('/peers')));
  assert.ok(themed.rows.every(row=>row.industry==='양자컴퓨팅'));
