(function(){
  ...
  function render(payload){
    ...
    const equityRiskOn=latest.equity<=.55,equityRiskOff=latest.equity>=.75,indexHedgeBuilding=deviation!==null?deviation>=10:latest.index>=.9,indexElevated=deviation!==null?deviation>=20:latest.index>=1.1,indexSpike=deviation!==null?deviation>=40:latest.index>=1.2;
    let state='중립',note='개별주식 심리와 지수 헤지 수요가 기준 범위에 있습니다.',tone='';
    if(equityRiskOff&&(indexElevated||latest.index>=1.1)){state='위험회피';note='개별주식 방어 심리와 지수 헤지가 함께 높습니다.';tone='negative';}else if(equityRiskOn&&indexHedgeBuilding){state='경계 — 개별주 Risk-on / 지수 헤지 증가';note='개별 종목 옵션은 위험선호지만, 최근 지수 풋 수요가 20일 평균보다 늘었습니다. 시장 하락 임박 신호라기보다 포트폴리오 헤지 증가로 해석합니다.';tone='caution';}else if(equityRiskOn){state='위험선호';note='개별주식 옵션 흐름은 위험선호 방향입니다.';tone='positive';}else if(indexSpike){state='헤지 급등';note='Index P/C가 평소보다 급등해 지수 방어 수요가 강합니다.';tone='negative';}else if(indexElevated){state='중립 · 헤지 증가';note='시장 전반은 중립이지만 지수 헤지 수요가 상승했습니다.';tone='caution';}
    byId('equityPcValue').textContent=latest.equity.toFixed(2);byId('indexPcValue').textContent=latest.index.toFixed(2);byId('equityPcReading').textContent=(equityRiskOn?'낙관적 · 위험선호':equityRiskOff?'방어적 · 위험회피':'중립 범위')+' · '+dayLabel(latest.date);byId('indexPcReading').textContent=(indexSpike?'헤지 급등':indexHedgeBuilding?'최근 헤지 증가':'보통 범위')+' · '+dayLabel(latest.date);byId('indexPcDeviation').textContent=deviation===null?'축적 중':(deviation>=0?'+':'')+deviation.toFixed(1)+'%';byId('indexPcAverage').textContent=average===null?'20일 데이터 축적 중 · '+dayLabel(latest.date):'20일 평균 '+average.toFixed(2)+' · '+dayLabel(latest.date);
